...
async def handle_private_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Фото/відео/GIF в приваті — приймається лише на кроці 'media'."""
...
async def finalize_ticket(update: Update, context: ContextTypes.DEFAULT_TYPE, from_callback=False):
    ...
    try:
        if media_type == "photo" and media_id:
            ...
            await bot.send_animation(chat_id=TECH_CHAT_ID, animation=media_id, caption=caption, parse_mode="Markdown")
        elif media_type == "document" and media_id:
            await bot.send_document(chat_id=TECH_CHAT_ID, document=media_id, caption=caption, parse_mode="Markdown")
        else:
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.ChatType.PRIVATE & (filters.PHOTO | filters.VIDEO | filters.ANIMATION | filters.Document.ALL),
